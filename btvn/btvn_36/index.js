...
btnStart.addEventListener("click", function () {
  ...
  const animationCountdown = setInterval(function () {
    if (numberCountdown.innerText === "Go!") {
      ...
      const currentQuestion = calcPoint.querySelector(
        ".number-question p span:first-child"
      );
      const totalQuestion = calcPoint.querySelector(
        ".number-question p span:last-child"
      );
      ...
      const checkQuestions = setInterval(function () {
        ...
        if (questions) {
          ...
          function renderQuestion(index) {
            ...
            if (infoQuestion.choose === 1)
              question.setAttribute("data-choose", 1);
            else question.setAttribute("data-choose", 2);
            question.innerHTML = `
                            <div class="heading-question">
                                <h2>${infoQuestion.question}</h2>
                                <p>${
                                  infoQuestion.choose === 1
                                    ? ""
                                    : "Choose 2 correct answer(s)"
                                }</p>
                            </div>

                            <div class="answers">
                                <p class="answer" data-correct="${
                                  infoQuestion.answers[0].correct
                                }">${stripHtml(
              infoQuestion.answers[0].answer
            )}</p>
                                <p class="answer" data-correct="${
                                  infoQuestion.answers[1].correct
                                }">${stripHtml(
              infoQuestion.answers[1].answer
            )}</p>
                                <p class="answer" data-correct="${
                                  infoQuestion.answers[2].correct
                                }">${stripHtml(
              infoQuestion.answers[2].answer
            )}</p>
                                <p class="answer" data-correct="${
                                  infoQuestion.answers[3].correct
                                }">${stripHtml(
              infoQuestion.answers[3].answer
            )}</p>
                            </div>
                            `;
            updateProgress();
            ...
            setTimeout(function () {
              ...
              if (
                +number[1] === 0 &&
                +currentQuestion.innerText < questions.length
              ) {
                rerenderQuestion();
          ...
          question.addEventListener("click", function (e) {
            ...
            if (
              !canClick &&
              target.parentElement.parentElement.dataset.choose === "1"
            )
              return;
            else if (
              !canClick &&
              target.parentElement.parentElement.dataset.choose === "2" &&
              saveChoose === 2
            )
              return;
            else {
